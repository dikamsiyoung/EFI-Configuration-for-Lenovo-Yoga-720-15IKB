...
- #### Extensible Firmware Interface (EFI) 
  ...
  The EFI configuration for a successful macOS boot with OpenCore requires a `BOOT` folder containing BOOTx64.efi (a file that initializes the boot sequence) and an `OC` folder containing necessary OpenCore files to be loaded and their settings are defined in `Config.plist`. 
